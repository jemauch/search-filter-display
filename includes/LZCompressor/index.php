<?php

// Exit if accessed directly
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

require_once( SFD_DIR . 'includes/LZCompressor/LZContext.php' );
require_once( SFD_DIR . 'includes/LZCompressor/LZData.php' );
require_once( SFD_DIR . 'includes/LZCompressor/LZReverseDictionary.php' );
require_once( SFD_DIR . 'includes/LZCompressor/LZString.php' );
require_once( SFD_DIR . 'includes/LZCompressor/LZUtil.php' );
require_once( SFD_DIR . 'includes/LZCompressor/LZUtil16.php' );